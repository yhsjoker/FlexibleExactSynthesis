...
./build/fes_app generate --mode exhaustive --k 4 --num 1 \
  --out results_repo/activity_uniform_k4_num1
```
...
./build/fes_app generate --mode exhaustive --k 4 --num 1 \
  ...
  --out results_repo/activity_cartesian_k4_num1
```
...
./build/fes_app generate --mode exhaustive --k 4 --num 1 \
  ...
  --out results_repo/activity_explicit_k4_num1
```
...
The same modes are available through JSON config files; see
`docs/configuration.md` and the examples in `config/`.
